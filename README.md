...
- Click it to open a toggle. Flipping it edits `~/.config/fcitx5/conf/keyboard.conf` idempotently (only the `EnableLongPress` key — every other setting you already have is left untouched) and restarts Fcitx5 so the change actually takes effect.
...
scripts/toggle-longpress.sh enable|disable
      ↓
~/.config/fcitx5/conf/keyboard.conf  (EnableLongPress=True|False, idempotent edit)
      ↓
...
- Only the `EnableLongPress` key is managed. Candidate lists, selection modifiers, and per-app blacklists are configured by Fcitx5 itself, not by this plugin, in this version.
